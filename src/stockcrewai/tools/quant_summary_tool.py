...
class QuantSummaryToolInput(BaseModel):
    model_config = ConfigDict(extra="forbid")

    factor_ids: list[_NonEmptyString] = Field(
        min_length=1,
        description="要查询的已验证量化因子 ID 列表；只能查询当前运行的 allowlist。",
    )
...
class QuantSummaryTool(BaseTool):
    name: str = "get_quant_summary"
    description: str = (
        "查询当前运行 allowlist 内已验证的量化摘要；不联网、不计算、不修改状态。"
    )
    args_schema: Type[BaseModel] = QuantSummaryToolInput
    ...
    def _run(self, factor_ids: list[str]) -> Any:
        result = self._evidence_store.get_quant_summary(factor_ids)
        return _json_safe(result)


__all__ = ["QuantSummaryTool", "QuantSummaryToolInput"]
